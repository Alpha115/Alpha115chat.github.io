...
if($board['bo_gallery_height']==0) $board['bo_gallery_height']=400;
...
<? } ?>
	...
	<div class="ui-qna-list" <?if($board['bo_table_width']>0){?>style="max-width:<?=$board['bo_table_width']?><?=$board['bo_table_width']>100 ? "px":"%"?>;margin:0 auto;"<?}?>>
	<div class="ui-footer">
		<? if ($write_href) { ?>
...
<? } ?>
	...
	<div class="ui-header"><span>GUEST BOOK</span></div>
	<div class="ui-message" style="max-height:<?=$board['bo_gallery_height']?>px;">
		...
		<li> 
			...
			<form name="fboardlist" method="post" action="<?=$board_skin_url?>/password.php" style="margin:0">
				...
				<? } ?>
					...
					<strong>
						<? if ($update_href) { ?><a href="<? echo $update_href ?>">M</a><? } ?>
				...
				<div class="qna-content">
					...
					<? if(strstr($lists[$ii]['wr_option'], 'secret')) { ?>
						<span style="opacity:0.6;">비밀 메시지입니다.</span><br />
					<? } ?>
						<?= $lists[$ii]['content'] ?>
						<? echo $secret_msg; ?>
					<? } ?>
					...
					<div class="datetime">
					<?=date('m.d | H:i', strtotime($lists[$ii]['wr_datetime']))?>
			...
			<? 
				if(strstr($lists[$ii]['wr_option'], 'secret') && !$is_admin && !$is_open) { 
					...
					<div class="secret-comment"><p class="announce">비밀 메시지입니다.</p></div><?}
				} else { 
