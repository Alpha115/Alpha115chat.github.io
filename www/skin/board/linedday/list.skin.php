				...
				<li>
					<form name="fboardlist" method="post" action="<?=$board_skin_url?>/password.php" style="margin:0">
						...
						<div  class="memo-content content-area"> 
							...
							<? } ?>
								<span class="con">
								<i class="fas fa-bell"></i> <b><?= $lists[$ii]['content'] ?>::</b></span>
								
									
									<?		
			if($lists[$ii]['wr_3']=="days") {$plusdate='+'.$lists[$ii]['wr_2'].'days';}
			else if ($lists[$ii]['wr_3']=="weeks") {$plusdate ='+'.$lists[$ii]['wr_2'].'week';}
			else if ($lists[$ii]['wr_3']=="month"){$plusdate='+'.$lists[$ii]['wr_2'].'months';}
			else{$plusdate='';}
			if($lists[$ii]['wr_4']) {$enddate=strtotime($lists[$ii]['wr_4']);}
			else {$enddate = strtotime($lists[$ii]['wr_1'].$plusdate);}
			$date=date("U",$enddate); //목표 날짜의 유닉스타임스탬프를 구함 
			$today=time(); //현재의 유닉스타임스탬프를 구함 
			$day=($date-$today)/60/60/24; //몇일이지났는가를 계산
			$day=ceil($day);

			?>
			
					개시 날짜 <strong class="txt-point"><?=date("Y/m/d",strtotime($lists[$ii]['wr_1']))?></strong> /
					<? if($lists[$ii]['wr_2']) {?>마감 기한 <strong class="txt-point"><?=$lists[$ii]['wr_2']?></strong><?if($lists[$ii]['wr_3']=="days"){?>일<?}else if ($lists[$ii]['wr_3']=="weeks"){?>주<? }else if ($lists[$ii]['wr_3']=="month"){?>달<?}?> /<?}?>
					마감 예정일은 <strong class="txt-point"><?=date("Y/m/d",$enddate)?></strong> /
					D-<strong class="txt-point"><?=$day?></strong>
							...
							<p class="control"><?
							 if($is_comment_write) { 
								?><a href="javascript:comment_wri('comment_write', '<?=$lists[$ii]['wr_id']?>');"><i class="fas fa-plus-circle"></i></i></a><? 
							} if(($member['mb_id'] && ($member['mb_id'] == $lists[$ii]['mb_id'])) || $is_admin) {
								if($update_href){?><a href="<?=$update_href?>"><i class="fas fa-feather-alt"></i></a><?}
								?><a href="<?=$delete_href?>"><i class="fas fa-eraser"></i></a><? 
