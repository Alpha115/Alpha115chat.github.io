...
<form name="fwrite" id="fwrite" action="<?php echo $action_url ?>" onsubmit="return fwrite_submit(this);" method="post" enctype="multipart/form-data" autocomplete="off">
	...
	<div class="ui-write-box ui-text-area<?=$w=='u'? " update":"";?>">
		...
		<p>
		<input type="text" name="wr_1" value="<?php echo $write['wr_1'] ?>" class="frm-input date" size="10" maxlength="255" placeholder="개시 날짜" required>
		<input type="text" name="wr_2" value="<?php echo $write['wr_2'] ?>" class="frm-input" size="5" maxlength="255" placeholder="마감 기한">
		<select name="wr_3">
		<option value="days" <?=$write['wr_3'] == "days" ? "selected" : ""?>>일</option>
		<option value="weeks" <?=$write['wr_3'] == "weeks" ? "selected" : ""?>>주</option>
		<option value="month" <?=$write['wr_3'] == "month" ? "selected" : ""?>>달</option>
		</select>
		<input type="text" name="wr_4" value="<?php echo $write['wr_4'] ?>" class="frm-input date" size="10" maxlength="255" placeholder="마감 지정일(있다면)">
		<input type="text" name="wr_content" id="content" class="frm-input" required value="<?=$content?>" placeholder="메모">
