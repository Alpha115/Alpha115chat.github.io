...
<div class="item" id="log_<?=$list_item['wr_id']?>">
		...
		<div class="ui-pic <?=$no_member_class?>">
			...
			<?php if ($is_checkbox) { ?>
				...
				<p class="no">
					...
					<?if(strstr($list_item['wr_option'],"secret")){?>
					&nbsp;&nbsp;<span class="co-secret highlight">S</span>
					<?}else if ($list_item['wr_protect']!=''){?>
					&nbsp;&nbsp;<span class="co-secret highlight">P</span>
					<?}else if($list_item['wr_secret'] == '1' ){?>
				...
				<?if($member['mb_level']>=$board['bo_comment_level']){?>
					<a href="#" onclick="$(this).parents('.item').addClass('back').removeClass('front');return false;" class="open-comment">C</a>
				<?}?> 
			</div>
			<!-- // 로그 상단 영역 -->
	...
	<?if($list_item['wr_content']!=''||$list_item['wr_1']){?>
	...
	<div class="co-content">
		<?if($list_item['wr_1']){?>
			<p><span class="co-memo highlight">#memo</span>&nbsp;<?=$list_item['wr_1']?></p>
		<?}?>
		<? if ($list_item['wr_reply_more']==1){?>
		...
		<div class="original_comment_area<?=$list_item['wr_reply_more'] ? " re_more":"";?>">
			...
			// 코멘트 출력 부분	.
			$html = 0;
			if (strstr($list_item['wr_option'], 'html1'))
			$html = 1;
			else if (strstr($list_item['wr_option'], 'html2'))
			$html = 2;  		
			if($html){ 
	...
	<!--  로그 코멘트 부분 -->
		...
		<a href="#" class="close-comment" onclick="$(this).parents('.item').removeClass('back').addClass('front');return false;">X</a>
